...
public class HanziToPinyinUtil {
	...
	private static HanziToPinyinUtil chineseSpelling = new HanziToPinyinUtil();
	...
	public static String getFirstSelling(String chs) {
		String key, value;
		buffer = new StringBuilder();
		for (int i = 0; i < chs.length(); i++) {
			key = chs.substring(i, i + 1);
			if (key.getBytes().length == 3) {
				value = (String) convert(key).substring(0, 1);
				if (value == null) {
					value = "unknown";
				}
			} else {
	 ...
	 * @return 拼音
	 */
	public static String convertToFirstSpells(String chines) {
		chines = chines.replaceAll("[\\pP\\p{Punct}]","");//清除所有符号,只留下字母 数字  汉字  共3类.
		return getFirstSelling(chines);
	}

	/**
	 * 汉字转换位汉语拼音，取汉字的首拼。英文字符不变
	 * 
	 * @param chines
	 *            汉字
	 * @return 拼音
	...
	public static String convertToFirstSpell(String chines) {
		return getSelling(chines).substring(0, 1);
	}

	public static void main(String[] args) {
		System.out.println(HanziToPinyinUtil.convertToFirstSpells("光?笹竹"));
	}
