package com.bh.system.service.impl;

import com.bh.common.constants.ParamConstant;
import com.bh.common.result.PageResult;
import com.bh.common.result.RestResult;
import com.bh.common.result.ResultUtils;
import com.bh.common.utils.DateUtil;
import com.bh.model.dao.PropertyManagementMapper;
import com.bh.model.dao.VoiceLibraryMapper;
import com.bh.model.domain.Community;
import com.bh.model.domain.PropertyManagement;
import com.bh.model.domain.VoiceLibrary;
import com.bh.system.redis.JWTRedisDAO;
import com.bh.system.service.VoiceLibraryService;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

import java.util.Date;
import java.util.HashMap;
import java.util.List;
import java.util.Map;

/**
 * Created by Mr.BH
 */
@Service
@Transactional
public class VoiceLibraryServiceImpl implements VoiceLibraryService {

    @Autowired
    private VoiceLibraryMapper voiceLibraryMapper;

    @Autowired
    private JWTRedisDAO jwtRedisDAO;

    @Override
    public RestResult save(VoiceLibrary voiceLibrary) {

        Date date = DateUtil.getCurrentDate();

        voiceLibrary.setStatus(0);
        voiceLibrary.setDelStatus(0);
        voiceLibrary.setCreateTime(date);

        voiceLibraryMapper.insertSelective(voiceLibrary);

        if (null == voiceLibrary.getId()){
            throw new IllegalArgumentException();
        }

        return ResultUtils.success("保存成功");
    }

    @Override
    public RestResult delete(List<Integer> pkIds) {
        if(pkIds == null || pkIds.isEmpty()){
            return ResultUtils.error(1,"没有选择需要删除的数据");
        }

        Integer[] ids = new Integer[pkIds.size()];

        if (pkIds.size() > 0){
            for (int i = 0; i < pkIds.size(); i++) {
                ids[i] = pkIds.get(i);
            }
        }

        // 小区信息 逻辑删除
        voiceLibraryMapper.deleteBatch(ids);
        return ResultUtils.success("删除成功");
    }

    @Override
    public RestResult update(VoiceLibrary voiceLibrary) {

        Date currentDate = DateUtil.getCurrentDate();

        voiceLibrary.setUpdateTime(currentDate);

        voiceLibraryMapper.updateByPrimaryKeySelective(voiceLibrary);

        return ResultUtils.success("编辑成功");
    }

    @Override
    public RestResult get(Integer id) {
        return ResultUtils.success(voiceLibraryMapper.selectByPrimaryKey(id));
    }

    @Override
    public RestResult findByVoiceType(Integer voiceType) {
        List<VoiceLibrary> vlList = voiceLibraryMapper.findByVoiceType(voiceType);
        if(vlList.isEmpty()){
            return ResultUtils.success("无数据");
        }
        return ResultUtils.success(vlList.get(0));
    }

    @Override
    public RestResult list(String keyword, Integer pageNum, Integer pageSize) {

        int n = pageNum == 1 ? pageNum = 0 : (pageNum = (pageNum - 1) * pageSize);

        // 查询参数
        Map domainMap = new HashMap();
        domainMap.put(ParamConstant.KEY_WORD,keyword);
        domainMap.put(ParamConstant.PAGE_SIZE,pageSize);
        domainMap.put(ParamConstant.PAGE_NUM,pageNum);

        List<Map> maps = voiceLibraryMapper.listPage(domainMap);
        int count = voiceLibraryMapper.countPage(domainMap);

        return ResultUtils.success(new PageResult<Map>(maps,count));
    }

    @Override
    public RestResult setStatus(Integer pkId){
        VoiceLibrary voiceLibrary = voiceLibraryMapper.selectByPrimaryKey(pkId);
        if(voiceLibrary.getStatus() == 0){// 设置禁用
            voiceLibrary.setStatus(1);
        }else{ //设置启用
            voiceLibrary.setStatus(0);
        }
        voiceLibrary.setUpdateTime(DateUtil.getCurrentDate());
        voiceLibraryMapper.updateByPrimaryKeySelective(voiceLibrary);

        return ResultUtils.success(voiceLibrary);
    }
}
